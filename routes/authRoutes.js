...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    const filename =
      `${req.body?.name.replace(/\s+/g, "")}-image-${Date.now()}` +
...
router.post(
  ...
  [
    ...
    validateFields.validName(
      ["name"],
      ["El nombre solo debe contener letras alfabeticas."]
    ),
    catchErrors,
  ],
  createUser
...
router.post(
  ...
  [
    ...
    validateFields.isEmail(),
    check("password", "La contraseña debe tener más de 8 caracteres.").isLength(
      { min: 8 }
    ),
    catchErrors,
  ],
